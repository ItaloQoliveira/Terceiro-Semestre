...
void ordena(carta baralho[52]){
    ...
    for(i=51;i>0;i--){
        maiorindex=i;
        for(j=0;j<=i;j++){
            if(baralho[j].num<baralho[maiorindex].num){
                maiorindex=j;
            }else if (baralho[j].num==baralho[maiorindex].num){
                if(baralho[j].naipe<baralho[maiorindex].naipe){
                    maiorindex=j;
                }
            }
            
        }
        troca(&baralho[maiorindex],&baralho[i]);
    }        
